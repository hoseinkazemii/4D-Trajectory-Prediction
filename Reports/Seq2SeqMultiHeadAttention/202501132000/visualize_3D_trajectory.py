...
csv_file = "Results_TestSet_1.csv"  # Replace with the path to your CSV file
data = pd.read_csv(csv_file)

# Extract the true and predicted trajectories
X_true, Y_true, Z_true = data["X_true"], data["Y_true"], data["Z_true"]
print(f"X_true: {np.array(X_true)}")
print(f"Y_true: {np.array(Y_true)}")
print(f"Z_true: {np.array(Z_true)}")
raise ValueError
