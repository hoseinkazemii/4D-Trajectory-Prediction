...
def _make_decoder_inputs(y_array):
    """
    y_array shape: (batch_size, pred_horizon, in_dim)
    Returns a 'decoder_inputs' array of the same shape,
    where decoder_inputs[:, t, :] = y_array[:, t-1, :] for t >= 1,
    and zeros for t=0. (Teacher forcing)
    """
    decoder_in = np.zeros_like(y_array)
    decoder_in[:, 1:, :] = y_array[:, :-1, :]
    return decoder_in
...
def _train_and_evaluate_model(split_data_dict, scalers_dict, row_counts, **params):
    ...
    split_data_dict: dict of { coordinate_str: dict(...) }
      Each coordinate_str maps to:
        {
          "X_train": np.array,  # shape (N, sequence_length, in_dim)
          "y_train": np.array,  # shape (N, prediction_horizon, in_dim)
          "X_val":   np.array,
          "y_val":   np.array,
          "X_test":  np.array,
          "y_test":  np.array
    ...
    for coord_str in coordinates:
        ...
        train_X = split_data_dict[coord_str]["X_train"]  # (N, sequence_length, in_dim)
        train_y = split_data_dict[coord_str]["y_train"]  # (N, prediction_horizon, in_dim)
        ...
        # --------------------------------------------------------------------
        # Build the "decoder inputs" using teacher forcing
        # We'll shift y by 1 along time dimension to get the previous step
        # as the decoder input.
        train_dec_X = _make_decoder_inputs(train_y)
        val_dec_X   = _make_decoder_inputs(val_y)
        test_dec_X  = _make_decoder_inputs(test_y)  # for test, this means teacher forcing in test as well
        # --------------------------------------------------------------------

        # Fit
        history = model.fit(
            [train_X, train_dec_X],  # two inputs
            train_y,                 # target
            validation_data=([val_X, val_dec_X], val_y),
            epochs=num_epochs,
            batch_size=batch_size,
            verbose=1 if verbose else 0
        )

        # Predict on test (teacher forcing for test)
        y_pred_test = model.predict([test_X, test_dec_X])
