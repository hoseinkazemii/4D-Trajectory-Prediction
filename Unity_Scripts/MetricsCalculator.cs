...
public class Metrics_Calculator : MonoBehaviour
{
    ...
    public Transform grabbableObjectBlindArea; // Assign the grabbable object in the Inspector
    ...
    public bool isBlindPickScenario = false; // Controlled by VRMainMenuManager
    ...
    {
        float sway = 0f;
        // Determine the active grabbable object based on the scenario
        Transform activeGrabbableObject = isBlindPickScenario ? grabbableObjectBlindArea : grabbableObject;

        if (activeGrabbableObject != null && activeGrabbableObject.parent == craneHook)
        {
            // Calculate sway based on the active grabbable object
            sway = Vector3.Distance(activeGrabbableObject.position, loadSwayMeasurementReferencePoint.position);
            loadSwayMeasurements.Add(sway);
        }

        // Update the centralized CSV writer with the latest metrics
        csvWriter.UpdateMetrics(Time.time, activeGrabbableObject.position.x, activeGrabbableObject.position.y, activeGrabbableObject.position.z, sway, loadingStarted);
    }
    ...
    void FixedUpdate()
    {
        ...
        taskEnded = DustCloudTrigger.taskEnded;  // Use taskEnded from DustCloudTrigger
    ...
    public void ExportMetrics()
    {
        ...
        using (StreamWriter writer = new StreamWriter(metricsFilePath))
        {
            ...
            {
                Transform activeGrabbableObject = isBlindPickScenario ? grabbableObjectBlindArea : grabbableObject;
                if (activeGrabbableObject != null)
                {
                    preciseness = Vector3.Distance(activeGrabbableObject.position, unloadingArea.position) / 5; // Divided by 5 because of scaling differences
                }
                else
                {
                    Debug.LogError("Active grabbable object is not assigned!");
                }
            }
            else
            {
                Debug.LogError("Unloading area is not assigned!");
            }
